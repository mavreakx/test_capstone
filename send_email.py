...
def extract_summary(file_path="data_input/data.csv", id=['936','1178']):
  ...
  campaigns['reporting_end'] = pd.to_datetime(campaigns['reporting_end'], dayfirst=True)
...
def compose_email(template, name, data_dict):
  ...
  composed = template.substitute(
    ...
    GITHUB_LINK='https://github.com/mavreakx/test-capstone'
  )
  return composed
...
def create_plot(file_path="data_input/data.csv", id=['936', '1178']):
  ...
  grouped = campaigns.groupby(by=['reporting_start', 'age', 'campaign_id'], as_index=False)['total_conversion'].sum()
